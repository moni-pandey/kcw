    ...
    $.ajax({
       ...
       // url: localStorage.getItem('webserviceurl') + "artist/importart",
        ...
        data: {
            artistID: localStorage.getItem('loggedINuserartistid'),
            artType: 'new_art3',
            artUrl: localStorage.getItem('uploadedimageurl'),
            //artUrl: encoded,
            socialMedia: 'upload',
...
function handle_errors(error) {
    switch (error.code) {
        case error.PERMISSION_DENIED:
            showAlert("user did not share geolocation data enable location service");
            break;

        case error.POSITION_UNAVAILABLE:
            showAlert("could not detect current position ,enable location service");
            break;

        case error.TIMEOUT:
            showAlert("retrieving position timed out ,enable location service");
            break;
        ...
        success: function(data) {
...

            {
                for (var k = 0; k < data.accounts.length; k++) {
                    if (socialMediaType == data.accounts[k].smAcctType) { // accflag=true
